            ...
            if itemName == "repair_kit" then
                ...
                if nearestCar ~= 0 then
                    if GetVehicleHealth(nearestCar) >= 5000 then
                        ...
                        CallRemoteEvent(player, "loadingbar:show", _("repairing"), Config.repairKitTime)-- LOADING BAR
                        Delay(Config.repairKitTime * 1000, function()
                            SetVehicleHealth(nearestCar, GetVehicleHealth(nearestCar) + Config.repairKitHealth)   
            ...
            if itemName == "jerican" then
                ...
                if nearestCar ~= 0 then
                    if VehicleData[nearestCar].fuel >= 100 then
                        ...
                        SetPlayerAnimation(player, "COMBINE")
                        SetPlayerBusy(player)
                        CallRemoteEvent(player, "loadingbar:show", _("refuel"), Config.gasStation.jericanTime)-- LOADING BAR
                        Delay(Config.gasStation.jericanTime * 1000, function()                            
                            VehicleData[nearestCar].fuel = VehicleData[nearestCar].fuel + Config.gasStation.jericanCapacity   
                            ...
                            SetPlayerAnimation(player, "STOP")
                            SetPlayerNotBusy(player)
                            CallRemoteEvent(player, "MakeNotification", _("car_refuelled_for", Config.gasStation.jericanCapacity, 'L'), "linear-gradient(to right, #00b09b, #96c93d)")
                        end)
                    end
                end
            end
